...
def calculate_battery_autonomy(isConnected, periodConnected, periodWeather, periodAdc, periodAccel, weatherOn, adcOn, accelOn, batteryLevel):
	...
	#Define variables as floating values
	#batteryLevel = 25.9
	batteryLev = 0.0
	batteryDiff = 0.0
	batteryExp = 0.0
	...
	batteryLevelVolts = 2+ float(batteryLevel)/100.0
	...
	batteryNominal = 11 #in mAh, assuming full capacity at +25oC
	batteryLev = float(batteryLevel)/100
	batteryDiff = 1- batteryLev
	batteryExp = 2*batteryLev   #exponent: approximates battery discharge behavior
	battCapacity = batteryNominal*(1-pow(batteryDiff, batteryExp)) #in mAh
